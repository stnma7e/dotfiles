...
#+BEGIN_SRC emacs-lisp
  ...
  (require 'google)
  (load-library "/google/src/cloud/itsanurag/personal-ws/google3/experimental/users/itsanurag/emacs-related/buganizer/buganizer.el")
  (require 'buganizer)
...
#+BEGIN_SRC emacs-lisp
  ...
  (load-file "~/.emacs.d/osc52.el")
  (setq-default interprogram-cut-function 'osc52-select-text)
...
#+BEGIN_SRC emacs-lisp
  ...
  (add-hook 'java-mode-hook
    #'(lambda ()
	(add-hook 'before-save-hook #'google-diformat-google-java-format-changed nil :local)))

  (defun jgit-java-format ()
    (interactive)
    (call-process "jgit-java-format"
	nil nil nil
	(buffer-file-name (window-buffer (minibuffer-selected-window))))
    (revert-buffer-quick))

  ;(defun jgit-java-format-region ()
  ;  (interactive)
  ;  (apply-function-to-region jgit-java-format-region-fun))

  ;(defun jgit-java-format-region-fun (text)
...
(defun yank-to-clipboard ()
  ...
  (interactive)
  (let ((tmx_tty (concat "/dev/" (shell-command-to-string "ps l | grep 'tmux' | grep -v grep | awk '{print $11}'")))
