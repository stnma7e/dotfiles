...
#+BEGIN_SRC emacs-lisp
  ...
  (add-hook 'kill-emacs-hook #'straight-freeze-versions)

  (use-package straight
    :custom
    ;; add project and flymake to the pseudo-packages variable so straight.el doesn't
    ;; download a different version than what eglot downloads.
    (straight-built-in-pseudo-packages '(emacs nadvice python image-mode project flymake xref eglot))
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package desktop
    ...
    (defun my/project-desktop-load ()
      "Select and load any saved project desktop with preview."
      (interactive)
      (let* ((desktops (directory-files desktop-dirname nil "\\.desktop$")))
        (if desktops
            (let* ((choice (completing-read "Load desktop: " desktops nil t))
                   (desktop-file (expand-file-name choice desktop-dirname))
                   (default-desktop (expand-file-name ".emacs.desktop" desktop-dirname)))
              (copy-file desktop-file default-desktop t)
              (desktop-read desktop-dirname)
              (message "Loaded desktop: %s" choice))
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package corfu
    ...
    (corfu-auto-prefix 2)      ; Minimum length of prefix for auto completion (adjust as needed)
    (corfu-auto-delay 0.1)     ; Delay in seconds before the popup appears (adjust as needed)
    :init
    (global-corfu-mode))
                                ...
                                (:settings
                                 (:java
                                  (:format
                                   (:enabled t
                                    :settings
                                    (:url ,(concat "file://" (expand-file-name "eclipse-java-google-style.xml" user-emacs-directory))
                                     :profile "GoogleStyle"))))))))
    (setq electric-indent-inhibit t)
    :hook ((java-mode java-ts-mode go-mode rust-mode python-mode) . eglot-ensure)
...
#+BEGIN_SRC emacs-lisp
  (defvar my/daily-log-dir "~/logs/")

  (defun my/daily-log-file ()
    (expand-file-name
     (format-time-string "%Y-%m-%d.org")
     my/daily-log-dir))

  (defun my/daily-log-open ()
    (interactive)
    (find-file (my/daily-log-file)))

  (defun my/daily-log-breadcrumb (text)
    (interactive "sNote: ")
    (with-current-buffer (find-file-noselect (my/daily-log-file))
      (goto-char (point-max))
      (unless (search-backward "* Breadcrumbs" nil t)
        (goto-char (point-max))
        (insert "\n* Breadcrumbs\n"))
      (goto-char (point-max))
      (search-backward "* Breadcrumbs" nil t)
      (next-line)
      (newline)
      (previous-line)
      (insert (format "%s — %s"
...
#+BEGIN_SRC emacs-lisp
  (use-package general
    ...
    (my/leader-keys
       ...
       "p l"  'my/project-desktop-load
       "p L"  'my/project-desktop-read

       "l b"  'my/daily-log-breadcrumb
       "l o"  'my/daily-log-open
    )
    (general-define-key
